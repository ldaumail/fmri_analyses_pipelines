...
for area in ['V1_lh','V1_rh', 'V2_lh', 'V2_rh', 'V3_lh', 'V3_rh', 'hV4_lh', 'hV4_rh', 'V3a_lh', 'V3a_rh', 'V3b_lh', 'V3b_rh']: #['V1', 'V2', 'V3', 'hV4', 'V3a', 'V3b']:  # ,
	for zstat in ['zstat2', 'zstat3']: #'zstat4'
		print(f'{datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")} | Area: {area} | zstat: {zstat}')
		os.makedirs(os.path.join('/Users/tonglab/Documents/Loic/phantom_mri_data/analysis/loic_anal/sub-F019_230321/data/functional/phantom/allRuns/topUp/noB0/psc/improved_roi'), exist_ok=True)
		resultsFile = f'/Users/tonglab/Documents/Loic/phantom_mri_data/analysis/loic_anal/sub-F019_230321/data/functional/phantom/allRuns/topUp/noB0/psc/improved_roi/timeseries_{area}_{zstat}.csv'
					...
					for subject in experiment['scanInfo'][fieldStrength].keys():
						for session in experiment['scanInfo'][fieldStrength][subject].keys():
							...
							for run in range(len(experiment['scanInfo'][fieldStrength][subject][session]['funcScans'][scan])):
								...
								masks = sorted(glob.glob(os.path.join(sessDir, f'data/functional/localizer/run01/outputs/topUp/noB0/masksNative/improved_masks/{area}_masks/{zstat}_{area}*.nii.gz')))
								os.makedirs(os.path.join(runDir, 'timeseries/improved_masks'), exist_ok=True)
								for mask in masks:
									maskName = os.path.basename(mask)[:-7]
									print(maskName)

									# get mean time-series of mask for whole scan
									if not os.path.exists(f'{runDir}/timeseries/improved_masks/{maskName}.txt') or overwrite:
										os.system(f'fslmeants -i {runDir}/timeseries/filtered_func_percentSC -o {runDir}/timeseries/improved_masks/{maskName}.txt -m {mask}')

									f = open(f'{runDir}/timeseries/improved_masks/{maskName}.txt')
