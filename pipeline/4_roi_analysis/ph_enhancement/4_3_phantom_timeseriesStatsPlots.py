...
for area in 'V1_lh','V1_rh', 'V2_lh', 'V2_rh', 'V3_lh', 'V3_rh', 'hV4_lh', 'hV4_rh', 'V3a_lh', 'V3a_rh', 'V3b_lh', 'V3b_rh': #['V1', 'V2', 'V3', 'hV4', 'V3a', 'V3b']:
	for z, zstat in enumerate(['zstat2']): # 'zstat3'
		dataFile = f'/users/tonglab/Documents/Loic/phantom_mri_data/analysis/loic_anal/sub-F019_230321/data/functional/phantom/allRuns/outputs/topUp/noB0/psc/phEnhancement_roi/timeseries_{area}_ph{zstat}.csv'
		...
		for row in data.index:
			regions.append("".join([data['region'][row].split(sep='_')[2],'_',data['region'][row].split(sep='_')[3]]))
				...
				for HRFmodel in ['doubleGamma']:#, 'singleGamma', 'singleGamma034']:
					...
					for subject in list(experiment['scanInfo'][fieldStrength].keys()):
						for session in experiment['scanInfo'][fieldStrength][subject].keys():
							...
							outDir = os.path.join('/Users/tonglab/Documents/Loic/phantom_mri_data/analysis/loic_anal/sub-F019_230321/analysis/results', topup, b0, HRFmodel, subject, session, scan, 'enhancement_roi')
